...
def kelvHelm(U,F,xPoints,yPoints,gamma,p0,rho1,rho2,u1,u2,delta,w0):
	...
	U[:,:,0,0] = rhos[:,None]
	U[:,:,0,1] = rhos[:,None] * us[:,None]
	U[:,:,0,2] = rhos[:,None] * vs[None,:]
	U[:,:,0,3] = energyFunc(p0,gamma,rhos[:,None],us[:,None],vs[None,:])
	
	F[:,:,0,0] = rhos[:,None] * us[:,None]
	F[:,:,0,1] = rhos[:,None] * us[:,None]**2 + p0
	F[:,:,0,2] = rhos[:,None] * us[:,None] * vs[None,:]
	F[:,:,0,3] = (energyFunc(p0,gamma,rhos[:,None],us[:,None],vs[None,:]) + p0) *us[:,None]
	
	#plt.imshow(U[:,:,0,3],interpolation='gaussian')
	#plt.show()
	return U, F
	
def kelvHelm2(U,F,xPoints,yPoints,gamma,p0,rho0,w0,a,n):
	# Initial conditions for Kelvin-Helmholtz instability
	Lx = xPoints[-1] - xPoints[0] 	
	Ly = yPoints[-1] - yPoints[0]
	Nx = len(xPoints)
	...
	rhos = rho0
	us   = w0*np.exp(-xPoints[:,None]**2/a**2)*np.sin(2*np.pi*n*yPoints[None,:]/L)
	vs   = np.zeros(Nx)
	vs[0:Nx/2] = 0.1
	vs[Nx/2:] = -0.1
	...
	U[:,:,0,1] = rhos * us
	U[:,:,0,2] = rhos * vs[:,None]
	U[:,:,0,3] = energyFunc(p0,gamma,rhos,us,vs[:,None])
	
	F[:,:,0,0] = rhos * us
	F[:,:,0,1] = rhos * us**2 + p0
	F[:,:,0,2] = rhos * us* vs[:,None]
	F[:,:,0,3] = (energyFunc(p0,gamma,rhos,us,vs[:,None]) + p0) *us
