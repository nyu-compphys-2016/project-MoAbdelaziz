...
def fluxUpdate(U,F,n):
	...
	newRho  = U[2:-2,2:-2,n+1,0]
	newRhoV = U[2:-2,2:-2,n+1,1]
	newV    = newRhoV/newRho
	newE    = U[2:-2,2:-2,n+1,2]
	newP    = (gamma-1) * (newE - 0.5*newRho*newV**2)
	
	F[2:-2,2:-2,n+1,0] = newRho*newV
	F[2:-2,2:-2,n+1,1] = newRho*newV**2 + newP
	F[2:-2,2:-2,n+1,2] = (newE+newP)*newV
	return F
...
U[:,:,0,0] = rho0
U[:,:,0,1] = rho0*v0
U[:,:,0,2] = energyFunc(p0,gamma,rho0,v0)

F[:,:,0,0] = rho0*v0
F[:,:,0,1] = rho0*v0**2 + p0
F[:,:,0,2] = (energyFunc(p0,gamma,rho0,v0) + p0)*v0
...
for n in range(Nt-1):
	...
	#print n, 'Second'
	#U2[2:-2,n,:]  = (3./4.)*U[2:-2,n,:] + (1./4.)*U1[2:-2,n,:] + (1./4.)*dt*LFunc(U1,F1,dx,dy,n)
	#F2 = fluxUpdate(U2,F1,n-1)
	#print n, 'Final'
	#U[2:-2,n+1,:] = (1./3.)*U[2:-2,n,:] + (2./3.)*U2[2:-2,n,:] + (2./3.)*dt*LFunc(U2,F2,dx,dy,n)
...
ax.plot_surface(X[2:-2,2:-2],Y[2:-2,2:-2],F[2:-2,2:-2,-1,0], rstride=1, cstride=1,cmap=cm.coolwarm) #plots 
