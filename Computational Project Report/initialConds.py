...
def isen(t,U,F,xPoints,yPoints,gamma,sigma,alpha,x0,rho0,p0):
	# Create a 2d isentropic wave (dependent on radial distance r instead of linear distance x)
	def f(x,y):
		A = np.zeros([len(y),len(x)])
		for i in range(len(y)):
			for k in range(len(x)):
				if np.sqrt((x[k]-x0)**2+(y[i]-x0)**2) < sigma:
					A[i,k] = (1 - ((x[k] - x0)/sigma)**2 - ((y[i] - x0)/sigma)**2)**2
				else:
					A[i,k] = 0
		return A
	
	rho = rho0*(1+alpha*f(xPoints,yPoints))
	#rhoy = rho0*(1+alpha*f(yPoints))
	p = p0*(rho/rho0)**gamma
	#py = p0*(rhoy/rho0)**gamma
	
	cs = np.sqrt(gamma*p/rho)
	#csy = np.sqrt(gamma*py/rhoy)
	cs0 = np.sqrt(gamma*p0/rho0) 

	v = 2./(gamma-1.) * (cs - cs0)
	vx = v*(xPoints[None,:]-x0) / np.sqrt((xPoints[None,:]-x0)**2+(yPoints[:,None]-x0)**2) 
	vy = v*(yPoints[:,None]-x0) / np.sqrt((xPoints[None,:]-x0)**2+(yPoints[:,None]-x0)**2) 
	#vy = 2./(gamma-1.) * (csy - cs0)	
	#surf = plt.imshow(f(xPoints,yPoints) , origin= 'lower')
	#plt.colorbar(surf)
	#plt.show()
